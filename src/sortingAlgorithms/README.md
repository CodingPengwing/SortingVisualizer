...
we're also not sure what you're doing reading this boring file). We recommend reading this while looking 
at some of the sorting algorithms to help you understand how we carry out animations for sorting. 
...
Animations happen after the sorting algorithm has finished running, hence why we keep a history.

- addToHistory() takes its own props (properties) as input. A props for addToHistory() would include 
the array and the sections we want to highlight.
